...
(define (racketlist->mupllist list)
  (cond [(pair? list) (apair (car list) (racketlist->mupllist (cdr list)))]
        [#t (aunit)]))

(define (mupllist->racketlist mupllist)
  (cond [(apair? mupllist) (cons (apair-e1 mupllist) (mupllist->racketlist (apair-e2 mupllist)))]
        [#t null]))
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
               ...
               (error "MUPL something is broken in ifgreater")))]
        [(fun? e)
         (closure env e)]
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
               ...
               (if (closure? v1)
                   (let* ([cf (closure-fun v1)]
                          [ce  (closure-env v1)]
                          [cfn (cons (fun-nameopt cf) v1)]
                          [cff (cons (fun-formal cf) v2)])
                   (eval-under-env (fun-body cf)
                                   (if (eq? (car cfn) #f)
                                       (cons cff ce)
                                       (cons cff (cons cfn ce)))))
                   (error "MUPL closure problem in call")))]
        [(mlet? e)
